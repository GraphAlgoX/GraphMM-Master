...
def rebuildroad(newfile, link2idx, nG):
    ...
    #         if len(data) != 0:
    #             link_id, s_node_id, e_node_id, link_dir, speed, vertex_count, points, neighbors = data[0]
    #             newlink_id = str(link2idx[link_id])
    #             neighbors_ls = neighbors.split(';')
    #             new_neighbors_ls = []
    #             for i in neighbors_ls:
    #                 nid = i.split(',')
    #                 nid[0] = str(link2idx[nid[0]])
    #                 new_neighbors_ls.append(','.join(nid))
    #             new_neighbors = ';'.join(new_neighbors_ls)

    #             newline = f'{newlink_id}\t{s_node_id}\t{e_node_id}\t{link_dir}\t{speed}\t{vertex_count}\t{points}|{new_neighbors}'
    #             fout.write(newline + '\n')
...
if __name__ == '__main__':
    ...
    json.dump(link2idx, open('link2idx.json', 'w'))
    # link2idx = json.load(open('/data/GeQian/g2s_2/GNNMapMatch_haixu/utils/link2idx.json','r'))
    rebuildroad('/data/GeQian/g2s_2/GNNMapMatch_haixu/data/newroad.txt', link2idx, nG)
    rebuildtrace('/data/GeQian/g2s_2/preprocessed_pure/full_trace.txt', '/data/GeQian/g2s_2/preprocessed_pure/full_trace_new.txt', link2idx, parent)
